...
func (m *AuditLogMiddleware) CreateMultipartUpload(ctx context.Context, bucketName storage.BucketName, key storage.ObjectKey, contentType *string, checksumType *string, tags map[string]string) (*storage.InitiateMultipartUploadResult, error) {
	start := time.Now()
	...
	res, err := m.Next.CreateMultipartUpload(ctx, bucketName, key, contentType, checksumType, tags)
	uid := ""
	if res != nil {
