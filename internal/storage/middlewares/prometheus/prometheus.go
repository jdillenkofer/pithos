...
func (psm *prometheusStorageMiddleware) CreateMultipartUpload(ctx context.Context, bucketName storage.BucketName, key storage.ObjectKey, contentType *string, checksumType *string, tags map[string]string) (*storage.InitiateMultipartUploadResult, error) {
	...
	err := psm.run(ctx, "PrometheusStorageMiddleware.CreateMultipartUpload", "CreateMultipartUpload", func(ctx context.Context) error {
		var err error
		result, err = psm.Next.CreateMultipartUpload(ctx, bucketName, key, contentType, checksumType, tags)
		return err
	})
	return result, err
}
