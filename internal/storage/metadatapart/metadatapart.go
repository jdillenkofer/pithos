...
func (mbs *metadataPartStorage) GetBucketPolicy(ctx context.Context, bucketName storage.BucketName) (string, error) {
	ctx, span := mbs.tracer.Start(ctx, "MetadataPartStorage.GetBucketPolicy")
	defer span.End()
	...
	policy, err := mbs.metadataStore.GetBucketPolicy(ctx, tx, bucketName)
	if err != nil {
		tx.Rollback()
		return "", err
	}
...
func (mbs *metadataPartStorage) PutBucketPolicy(ctx context.Context, bucketName storage.BucketName, policy string) error {
	ctx, span := mbs.tracer.Start(ctx, "MetadataPartStorage.PutBucketPolicy")
	defer span.End()
	...
	err = mbs.metadataStore.PutBucketPolicy(ctx, tx, bucketName, policy)
	if err != nil {
		tx.Rollback()
		return err
	}
...
func (mbs *metadataPartStorage) DeleteBucketPolicy(ctx context.Context, bucketName storage.BucketName) error {
	ctx, span := mbs.tracer.Start(ctx, "MetadataPartStorage.DeleteBucketPolicy")
	defer span.End()
	...
	err = mbs.metadataStore.DeleteBucketPolicy(ctx, tx, bucketName)
	if err != nil {
		tx.Rollback()
		return err
	}
