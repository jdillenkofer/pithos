...
type Repository interface {
	...
	ReleaseClaim(ctx context.Context, tx *sql.Tx, outboxID string, id ulid.ULID, owner string, nextAttemptAt time.Time, now time.Time) (bool, error)
...
const (
	...
	findFirstStmt    = "SELECT id, destination_arn, event_name, payload_format, payload, attempts, next_attempt_at, created_at, updated_at, claim_owner, claim_until, version FROM notification_outbox_entries WHERE outbox_id = $1 AND next_attempt_at <= $2 AND (claim_owner IS NULL OR claim_until <= $2) ORDER BY next_attempt_at ASC, id ASC LIMIT 1"
	claimStmt        = "UPDATE notification_outbox_entries SET claim_owner = $1, claim_until = $2, attempts = attempts + 1, version = version + 1, updated_at = $3 WHERE id = $4 AND outbox_id = $5 AND version = $6 AND next_attempt_at <= $7 AND (claim_owner IS NULL OR claim_until <= $7)"
	deleteClaimStmt  = "DELETE FROM notification_outbox_entries WHERE id = $1 AND outbox_id = $2 AND claim_owner = $3"
	releaseClaimStmt = "UPDATE notification_outbox_entries SET claim_owner = NULL, claim_until = NULL, next_attempt_at = $1, version = version + 1, updated_at = $2 WHERE id = $3 AND outbox_id = $4 AND claim_owner = $5"
...
func (r *SQLRepository) ReleaseClaim(ctx context.Context, tx *sql.Tx, outboxID string, id ulid.ULID, owner string, nextAttemptAt time.Time, now time.Time) (bool, error) {
	res, err := tx.ExecContext(ctx, releaseClaimStmt, nextAttemptAt, now, id.String(), outboxID, owner)
	if err != nil {
		return false, err
	}
	affected, err := res.RowsAffected()
