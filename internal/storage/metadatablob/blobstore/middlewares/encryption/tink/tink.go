...
func NewWithTPM(tpmPath string, persistentHandle uint32, innerBlobStore blobstore.BlobStore) (blobstore.BlobStore, error) {
	// Create TPM AEAD
	tpmAEAD, err := tpm.NewAEAD(tpmPath, persistentHandle)
	if err != nil {
		return nil, err
	}
