...
const (
	...
	findFirstStorageOutboxEntryForBucketStmt = "SELECT id, operation, bucket, key, content_type, data, created_at, updated_at FROM storage_outbox_entries WHERE bucket = ? ORDER BY id ASC LIMIT 1"
	findLastStorageOutboxEntryForBucketStmt  = "SELECT id, operation, bucket, key, content_type, data, created_at, updated_at FROM storage_outbox_entries WHERE bucket = ? ORDER BY id DESC LIMIT 1"
	insertStorageOutboxEntryStmt             = "INSERT INTO storage_outbox_entries (id, operation, bucket, key, content_type, data, created_at, updated_at) VALUES(?, ?, ?, ?, ?, ?, ?, ?)"
	updateStorageOutboxEntryByIdStmt         = "UPDATE storage_outbox_entries SET operation = ?, bucket = ?, key = ?, content_type = ?, data = ?, updated_at = ? WHERE id = ?"
