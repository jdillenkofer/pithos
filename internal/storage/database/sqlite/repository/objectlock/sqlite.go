...
// LockBucket must precede object and part locks. SQLite's writable transaction
// is already exclusive; PostgreSQL holds a row lock until transaction end.
func (r *repository) LockBucket(ctx context.Context, tx *sql.Tx, name metadatastore.BucketName) error {
	var found string
	err := tx.QueryRowContext(ctx, "SELECT name FROM buckets WHERE name = $1", name.String()).Scan(&found)
...
func (r *repository) LockObject(ctx context.Context, tx *sql.Tx, id ulid.ULID) error {
	var found string
	return tx.QueryRowContext(ctx, "SELECT id FROM objects WHERE id = $1", id.String()).Scan(&found)
}
