CREATE TABLE bucket_object_lock_configurations (
  bucket_name TEXT NOT NULL PRIMARY KEY REFERENCES buckets(name) ON DELETE CASCADE,
  ...
  CHECK (
    ...
    OR
    (default_retention_mode IS NOT NULL AND default_retention_mode IN ('GOVERNANCE', 'COMPLIANCE') AND
