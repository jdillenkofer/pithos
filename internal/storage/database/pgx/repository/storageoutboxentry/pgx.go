...
const (
	...
	upsertStorageOutboxEntryTagStmt                               = "INSERT INTO storage_outbox_entry_tags (outbox_entry_id, key, value) VALUES($1, $2, $3) ON CONFLICT (outbox_entry_id, key) DO UPDATE SET value = EXCLUDED.value"
	...
	upsertStorageOutboxEntryUserMetadataStmt                      = "INSERT INTO storage_outbox_entry_user_metadata (outbox_entry_id, key, value) VALUES($1, $2, $3) ON CONFLICT (outbox_entry_id, key) DO UPDATE SET value = EXCLUDED.value"
...
func (sor *pgxRepository) FindFirstGlobalStorageOutboxEntry(ctx context.Context, tx *sql.Tx, outboxId string) (*storageoutboxentry.Entity, error) {
	row := tx.QueryRowContext(ctx, findFirstGlobalStorageOutboxEntryStmt, outboxId)
	return convertRowToStorageOutboxEntryEntity(row)
}

func (sor *pgxRepository) FindLastGlobalStorageOutboxEntry(ctx context.Context, tx *sql.Tx, outboxId string) (*storageoutboxentry.Entity, error) {
	row := tx.QueryRowContext(ctx, findLastGlobalStorageOutboxEntryStmt, outboxId)
	return convertRowToStorageOutboxEntryEntity(row)
}

func (sor *pgxRepository) FindFirstGlobalStorageOutboxEntryForBucket(ctx context.Context, tx *sql.Tx, outboxId string, bucketName storage.BucketName) (*storageoutboxentry.Entity, error) {
	row := tx.QueryRowContext(ctx, findFirstGlobalStorageOutboxEntryForBucketStmt, outboxId, bucketName.String())
	return convertRowToStorageOutboxEntryEntity(row)
}

func (sor *pgxRepository) FindLastGlobalStorageOutboxEntryForBucket(ctx context.Context, tx *sql.Tx, outboxId string, bucketName storage.BucketName) (*storageoutboxentry.Entity, error) {
...
func (sor *pgxRepository) SaveStorageOutboxEntryCreateBucketOptions(ctx context.Context, tx *sql.Tx, outboxId string, id ulid.ULID, options storageoutboxentry.CreateBucketOptions) error {
	_, err := tx.ExecContext(ctx, `INSERT INTO storage_outbox_entry_create_bucket_options (outbox_entry_id, owner_account_id) SELECT id, $1 FROM storage_outbox_entries WHERE id = $2 AND outbox_id = $3 ON CONFLICT (outbox_entry_id) DO UPDATE SET owner_account_id = EXCLUDED.owner_account_id`, options.OwnerAccountID, id.String(), outboxId)
	return err
}

func (sor *pgxRepository) FindStorageOutboxEntryCreateBucketOptionsById(ctx context.Context, tx *sql.Tx, outboxId string, id ulid.ULID) (*storageoutboxentry.CreateBucketOptions, error) {
...
func (sor *pgxRepository) SaveStorageOutboxEntryPutOptions(ctx context.Context, tx *sql.Tx, outboxId string, id ulid.ULID, putOptions *storageoutboxentry.PutOptions) error {
...
func (sor *pgxRepository) FindStorageOutboxEntryPutOptionsById(ctx context.Context, tx *sql.Tx, outboxId string, id ulid.ULID) (*storageoutboxentry.PutOptions, error) {
...
func (sor *pgxRepository) ClaimFirstStorageOutboxEntry(ctx context.Context, tx *sql.Tx, outboxId string, owner string, now time.Time, claimUntil time.Time) (*storageoutboxentry.Entity, bool, error) {
...
func (sor *pgxRepository) DeleteStorageOutboxEntryByClaimOwner(ctx context.Context, tx *sql.Tx, outboxId string, id ulid.ULID, owner string) (bool, error) {
...
func (sor *pgxRepository) ReleaseStorageOutboxEntryClaim(ctx context.Context, tx *sql.Tx, outboxId string, id ulid.ULID, owner string, now time.Time) (bool, error) {
...
func (sor *pgxRepository) ExtendStorageOutboxEntryClaim(ctx context.Context, tx *sql.Tx, outboxId string, id ulid.ULID, owner string, now time.Time, claimUntil time.Time) (bool, error) {
