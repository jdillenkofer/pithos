...
func (wor *pgxRepository) Count(ctx context.Context, tx *sql.Tx, outboxId string) (int, error) {
	var count int
...
func (wor *pgxRepository) FindFirstWebhookOutboxEntry(ctx context.Context, tx *sql.Tx, outboxId string) (*webhookoutboxentry.Entity, error) {
	row := tx.QueryRowContext(ctx, findFirstWebhookOutboxEntryStmt, outboxId)
	return convertRowToWebhookOutboxEntryEntity(row)
}

func (wor *pgxRepository) SaveWebhookOutboxEntry(ctx context.Context, tx *sql.Tx, outboxId string, webhookOutboxEntry *webhookoutboxentry.Entity) error {
...
func (wor *pgxRepository) ClaimFirstWebhookOutboxEntry(ctx context.Context, tx *sql.Tx, outboxId string, owner string, now time.Time, claimUntil time.Time) (*webhookoutboxentry.Entity, bool, error) {
...
func (wor *pgxRepository) DeleteWebhookOutboxEntryByClaimOwner(ctx context.Context, tx *sql.Tx, outboxId string, id ulid.ULID, owner string) (bool, error) {
...
func (wor *pgxRepository) ReleaseWebhookOutboxEntryClaim(ctx context.Context, tx *sql.Tx, outboxId string, id ulid.ULID, owner string, now time.Time) (bool, error) {
...
func (wor *pgxRepository) ExtendWebhookOutboxEntryClaim(ctx context.Context, tx *sql.Tx, outboxId string, id ulid.ULID, owner string, now time.Time, claimUntil time.Time) (bool, error) {
