...
func TestPolicyReloadRejectsEmptyConditionBodies(t *testing.T) {
	runIntegrationTest(t, func(t *testing.T, suffix string, db database.DatabaseType, path, replicated, filesystem bool, encryption storageFactory.EncryptionType, outbox, compression bool) {
		t.Run(suffix, func(t *testing.T) {
			...
			valid := `{"schemaVersion":1,"policies":{"admin":{"Version":"2012-10-17","Statement":{"Effect":"Allow","Action":"s3:*","Resource":"*"}},"p":{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Action":["s3:CreateBucket","s3:PutObject"],"Resource":"*"},{"Effect":"Allow","Action":"s3:GetObject","Resource":"*","Condition":{"StringEquals":` + condition + `}}]}},"bindings":[{"policy":"admin","subjects":[{"type":"principal","accountId":"test-account","principalId":"test-principal"}]},{"policy":"p","subjects":[{"type":"principal","accountId":"test-account","principalId":"policy-test"}]}]}`
			...
			_, addr, cleanup := setupTestServerWithAuthorizer(authorizer, db, path, replicated, filesystem, encryption, outbox, compression)
			t.Cleanup(cleanup)
			client := setupS3ClientWithCredentials(testAPIEndpoint, addr, path, policyTestAccessKeyId, policyTestSecretAccessKey)
