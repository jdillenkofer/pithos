...
Requests that target an explicit object version through the `versionId` query parameter use `HeadObjectVersion`, `GetObjectVersion`, or `DeleteObjectVersion`. Bucket versioning configuration uses `GetBucketVersioning` and `PutBucketVersioning`; `GET ?versions` uses `ListObjectVersions`.
